...
!!! note
    The syntax for async flow may be too verbose for regular usage, especially when there are many async functions or complex logic inside. [Check out how](/api/#reactionscheduler) you can configure reaction scheduler to use microtasks, so you can write async flows without `utx`
...
Before dipole v2.5.0 changing reaction dependencies caused the reaction running in infinite loop:
...
Since v2.5.0 there is [`maxReactionIterations` option](/api/#maxreactioniterations) in global config, so by default it will break the execution of the endless loop after 100 iterations.
